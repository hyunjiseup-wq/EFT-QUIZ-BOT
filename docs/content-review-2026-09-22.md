...
- 보관 모드: 공통 449 / PvP 13 / PvE 2, 총 464문항.
  24차에서 공통 4문항 출제를 보류해 활성 460문항(공통 445 / PvP 13 / PvE 2),
  실제 출제 가능 PvP 458 / PvE 447이다.
...
- 24차(9월 23일): `e96413e` 이후 근거 미확보 4문항을 재검색했으나 현재 정답을 확정하지 못했다.
  Q95·102·225·226의 원문·ID를 보존하고 출제를 보류했다. 보류·재활성화 기능과 회귀 검사를 추가했다.
- 근거를 기록한 문항: 460개(1차 20 + 2차 14 + 3차 19 + 4차 19 + 5차 32 + 6차 28 + 7차 15 + 8차 27 + 9차 14 + 10차 26 + 11차 8 + 12차 19 + 13차 25 + 14차 22 + 15차 35 + 16차 11 + 17차 8 + 18차 12 + 19차 15 + 20차 23 + 21차 42 + 22차 20 + 23차 6).
  그 외 4개는 확인 완료로 표시하지 않고 출제 보류했다. 근거가 있는 문항도 충돌·미확인 사항이 남는다.
- `volatile`: 290개(2차 5개, 4차 2개, 5차 17개, 6차 9개, 7차 1개, 8차 8개, 9차 3개, 11차 1개, 12차 1개, 14차 3개, 15차 4개, 16차 1개, 18차 9개, 19차 9개, 20차 14개, 21차 2개, 22차 4개 추가). 이 표시는 **출제 제외가 아니다**.
  실제 보류는 별도 `enabled: false`로 제어한다. Q95·102·225·226을 제외한 남은 검토 항목은
  활성 풀에서 출제될 수 있다. `volatile`만으로 출제를 제외하지는 않는다.
...
### 24차 — 근거 미확보 문항의 출제 보류 (2026-09-23)

본편 HUD·킬피드와 상인 배율의 최신 직접 근거를 재검색했으나 확인하지 못했다.
검색 결과에는 Arena 관전 UI, 외부 오버레이·치트 광고, 오래된 댓글·가이드가 혼재해
본편의 현행 규칙을 확인하는 근거로 채택하지 않았다. 23차 API 서버 오류와 1.1.0의
평균 매입가 조정만으로 개별 배율·순위를 확정할 수 없다는 한계도 해소되지 않았다.
이들 문항을 역사 퀴즈로 바꾸거나 근거 필드만 채워 검증 완료처럼 표시하지 않았다.

- Q95·102·225·226에 `enabled: false`와 구체적인 `disabled_reason`을 추가했다.
  문제·보기·정답 인덱스·해설·난이도·모드·기존 메모는 원문 그대로 보존했다.
- `enabled` 생략 시 활성이다. false인 문항은 보류 사유가 필수이며 문자열·숫자로
  적힌 불리언은 형식 오류다. 보류 문항도 중복 ID·정답 인덱스 등 전체 형식 검사를 받는다.
- 시작 시 검증·모드별 난이도 충분성은 활성 문항만 계산한다. 런타임 로더, 모드 필터,
  난이도 그룹 및 직접 추출 경로 모두 보류 문항을 제외한다.
- `check_questions.py --disabled`로 보류 문항·사유를 조회한다. `--volatile` 출력은
  출제 상태와 보류 사유를 표시하고, 검증 전 값에 '현재 정답'이라는 이름을 붙이지 않는다.
- 파일에는 464문항을 보관하되 런타임 활성 풀은 460문항이다. 대시보드·운영 점검의
  풀 크기도 활성 수를 사용한다. 한영 README에 보관 수와 실제 출제 수를 따로 표시했다.
- 검증 근거를 확보하고 내용 검수 후 `enabled`를 true로 바꾸거나 제거하면 복구할 수 있다.
  파일을 읽는 시점은 봇 시작 시점이다. 실행 중 봇이나 이미 시작한 세션을 중단하지 않았으며,
  변경을 운영에 적용하려면 재시작이 필요하다.

보류는 검증 완료를 대신하지 않는다. 이 네 문항을 재활성화하려면 다음 자료가 필요하다.
본편 최신 PMC UI에서 기본 지도·처치 로그·퀘스트 알림을 구분할 수 있는 자료,
그리고 1.1.0 이후 상인별 매입가/기본가격 배율을 비교할 수 있는 신뢰 가능한 자료다.
`.env`·운영 DB·DB 스키마·응시 기록·실행 중 봇은 변경하지 않았다.
...
- 23차는 회귀 테스트 3개 추가 후 전체 208개 통과. 464문항 형식 검사·Ruff·변경 테스트
  ...
  로컬 재실행하지 않았다.
- 24차는 회귀 테스트 10개 추가 후 전체 218개 통과. 464문항 형식 검사 및 보류 4문항 출력,
  변경 모듈·테스트 컴파일, `git diff --check` 통과. Ruff 줄 길이 1곳을 수정했다.
  600회 추출에서 보류 문항 제외·모드 격리·난이도 수량·ID 중복 없음을 확인했다.
  CLI·대시보드 테스트가 활성 460·PvP 458·PvE 447을 검사한다. 근거 460·변동형 290은 유지한다.
  임시 DB 합성 부하도 로컬 재실행했다. 8,500명·25,500회 완주·동시 랭킹 200회·동시 세션
  250개 및 초과 거절을 통과했다. 통계·랭킹·후보 약 0.075초, 동시 랭킹 약 0.141초이며
  실제 Discord 운영 성능 보장값은 아니다.
